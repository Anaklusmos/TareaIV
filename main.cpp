...
void instrucciones()
{
    SDL_Texture *menu_instrucciones = IMG_LoadTexture(renderer,"menu_instrucciones.png");
    SDL_Rect inst_rect;
    SDL_QueryTexture(menu_instrucciones, NULL, NULL, &inst_rect.w, &inst_rect.h);
    inst_rect.x = 0;
    inst_rect.y = 0;
    ...
    while(true)
    {
        ...
        // Clear the entire screen to our selected color.
        SDL_RenderClear(renderer);
        SDL_RenderCopy(renderer, menu_instrucciones, NULL, &inst_rect);
        SDL_RenderPresent(renderer);
        while(SDL_PollEvent(&Event))
        {
            if(Event.type == SDL_QUIT)
